...
const BASE_PORT: number = Number(process.env.PORT) || 3081;
...
if (cluster.isPrimary) {
  ...
  const discoverService = async (serviceName: string) => {
    try {
      ...
      console.log(`Discovered services for ${serviceName}:`, services[Math.floor(Math.random() * services.length)].ServiceID);
    } catch (error) {
